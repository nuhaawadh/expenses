    ...
    <header className="fixed inset-x-0 top-0 z-50">
      ...
      >
        <nav className="frame flex h-16 items-center justify-between md:h-[72px]" aria-label="Primary">
          <a href="#top" className="relative z-10" aria-label="HIGHLink — home" onClick={() => setOpen(false)}>
            <Logo />
          </a>
          ...
          >
            <div className="frame flex h-full flex-col justify-between pb-10 pt-10">
              <ul className="space-y-1">
                {[{ label: "Home", href: "#top" }, ...site.nav].map((l, i) => (
                  <motion.li
                    key={l.href + l.label}
                    initial={{ y: 24, opacity: 0 }}
                    animate={{ y: 0, opacity: 1 }}
                  ...
                  >
                    <a
                      ...
                      className="block py-2 text-[2.5rem] font-medium leading-tight tracking-[-0.04em]"
                    >
                      {l.label}
                    </a>
                  </motion.li>
