 ...
 */
export function BookingEmbed({ t }: { t: Dict["booking"] }) {
  if (site.bookingUrl) {
    return (
  ...
  return (
    <div className="relative overflow-hidden rounded-[14px] bg-ink-2/80 ring-1 ring-inset ring-line-2">
      <div className="flex items-center justify-between border-b border-line px-5 py-4 md:px-8">
        <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-mute">{t.panelLeft}</span>
        <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-dim">{t.panelRight}</span>
      </div>
      ...
      <div className="absolute inset-0 top-[53px] grid place-items-center bg-gradient-to-b from-transparent via-ink-2/60 to-ink-2/80">
        <div className="text-center">
          <p className="font-mono text-sm tracking-[0.14em] text-fg">{t.placeholder}</p>
          <p className="mt-3 text-[13px] text-dim">{t.placeholderNote}</p>
        </div>
      </div>
    </div>
  );
