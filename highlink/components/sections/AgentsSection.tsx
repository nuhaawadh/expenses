...
/** Agent tasks are illustrative UI states, not client data. */
export function AgentsSection({ t }: { t: Dict["agents"] }) {
  const AGENTS = t.items;
    ...
    <section className="section relative" aria-labelledby="agents-title">
      <div className="frame">
        ...
        <div ref={ref} className="relative mt-20 md:mt-28">
          {/* system bus */}
          <div aria-hidden className="absolute start-[11px] top-0 bottom-0 w-px bg-line-2 lg:start-0 lg:end-0 lg:top-[11px] lg:bottom-auto lg:h-px lg:w-auto">
            <motion.span
              className="absolute inset-0 origin-top bg-gradient-to-b from-accent/70 via-accent/30 to-transparent lg:origin-left lg:bg-gradient-to-r rtl:lg:origin-right rtl:lg:bg-gradient-to-l"
              initial={{ scale: 0 }}
          ...
          <ul className="grid gap-10 lg:grid-cols-5 lg:gap-6">
            {AGENTS.map((a, i) => {
              ...
              return (
                <motion.li
                  key={a.name}
                  className="relative ps-12 lg:ps-0 lg:pt-14"
                  initial={{ opacity: 0, y: 20 }}
                  animate={seen ? { opacity: 1, y: 0 } : {}}
                  transition={{ duration: 0.9, ease, delay: 0.3 + i * 0.12 }}
                >
                  <span aria-hidden className="absolute start-0 top-0 grid h-[23px] w-[23px] place-items-center">
